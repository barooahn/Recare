...
jQuery(document).ready(function ($) {
    ...
    $("#side-sortables").sortable({
        ...
        stop: function (e, ui) {
            ...
            $.post(ajaxurl, {
                ...
                page: page,
                tab: tab,
                action: "ninja_forms_side_sortable",
                nf_ajax_nonce: ninja_forms_settings.nf_ajax_nonce
            }, function (response) {
    ...
    $(".ninja-forms-delete-form").click(function (event) {
        ...
        if (answer) {
            $.post(ajaxurl, {
                form_id: form_id,
                action: "ninja_forms_delete_form",
                nf_ajax_nonce: ninja_forms_settings.nf_ajax_nonce
            }, function (response) {
    ...
    $(document).on('click', '.ninja-forms-field-add-list-option', function (event) {
        ...
        $.post(ajaxurl, {
            ...
            action: "ninja_forms_add_list_option",
            nf_ajax_nonce: ninja_forms_settings.nf_ajax_nonce
        }, function (response) {
    ...
    $(document).on('click', '.save-list-import', function (e) {
        ...
        $.post(ajaxurl, {
            options: options,
            field_id: field_id,
            action: "ninja_forms_import_list_options",
            nf_ajax_nonce: ninja_forms_settings.nf_ajax_nonce
        }, function (response) {
            $("#ninja_forms_field_" + field_id + "_list_options").append(response);
            tb_remove();
    ...
    $(document).on('change', '.ninja-forms-list-populate-term', function (e, data) {
        ...
        if (this.value != '') {
            $.post(ajaxurl, {
                ...
                tax_name: this.value,
                from_ajax: 1,
                action: "ninja_forms_list_terms_checkboxes",
                nf_ajax_nonce: ninja_forms_settings.nf_ajax_nonce
            }, function (response) {
    ...
    $(document).on('click', '.ninja-forms-field-add-calc', function (e) {
        ...
        $.post(ajaxurl, {
            field_id: field_id,
            x: x,
            action: "ninja_forms_add_calc_row",
            nf_ajax_nonce: ninja_forms_settings.nf_ajax_nonce
        }, function (response) {
    ...
    $('.nf-delete-on-uninstall-prompt').nfAdminModal({
        title: nf_nuke_title,
        buttons: '.nf-delete-on-uninstall-prompt-buttons'
    });
