...
function um_account_tab__delete($info)
{
    ...
    if ($output) { ?>
        ...
        <div class="um-account-heading uimob300-hide uimob500-hide"><i
                class="<?php echo $icon; ?>"></i><?php echo $title; ?></div>
        ...
        <div class="um-col-alt um-col-alt-b">
            <div class="um-left"><input type="submit" name="um_account_submit" id="um_account_submit"
                                        value="<?php _e('Delete Account', 'ultimatemember'); ?>" class="um-button"/>
            </div>
...
function um_account_tab__privacy($info)
{
    ...
    if ($output) { ?>
        ...
        <div class="um-account-heading uimob340-hide uimob500-hide"><i
                class="<?php echo $icon; ?>"></i><?php echo $title; ?></div>
        ...
        <div class="um-col-alt um-col-alt-b">
            <div class="um-left"><input type="submit" name="um_account_submit" id="um_account_submit"
                                        value="<?php _e('Update Privacy', 'ultimatemember'); ?>" class="um-button"/>
            </div>
...
function um_account_tab__general($info)
{
    ...
    if ($output) { ?>
        ...
        <div class="um-col-alt um-col-alt-b">
            <div class="um-left"><input type="submit" name="um_account_submit" id="um_account_submit"
                                        value="<?php _e('Update Account', 'ultimatemember'); ?>" class="um-button"/>
            </div>
...
function um_account_tab__password($info)
{
    ...
    if ($output) { ?>
        ...
        <div class="um-col-alt um-col-alt-b">
            <div class="um-left"><input type="submit" name="um_account_submit" id="um_account_submit"
                                        value="<?php _e('Update Password', 'ultimatemember'); ?>" class="um-button"/>
            </div>
...
function um_account_tab__notifications($info)
{
    ...
    if ($output) { ?>
        ...
        <?php if (class_exists('UM_Messaging_API') || class_exists('UM_Followers_API')) { ?>
            <div class="um-field">
                <div class="um-field-label"><label for=""><?php _e('Email me when', 'ultimatemember'); ?></label>
        ...
        <div class="um-col-alt um-col-alt-b">
            <div class="um-left"><input type="submit" name="um_account_submit" id="um_account_submit"
                                        value="<?php _e('Update Notifications', 'ultimatemember'); ?>"
                                        class="um-button"/></div>
...
function um_account_user_photo_hook__mobile($args)
{
    ...
    <div class="um-account-meta radius-<?php echo um_get_option('profile_photocorner'); ?> uimob340-show uimob500-show">
        ...
        <div class="um-account-meta-img"><a
                href="<?php echo um_user_profile_url(); ?>"><?php echo get_avatar(um_user('ID'), 120); ?></a></div>

        <div class="um-account-name">
            <a href="<?php echo um_user_profile_url(); ?>"><?php echo um_user('display_name'); ?></a>

            <div class="um-account-profile-link"><a href="<?php echo um_user_profile_url(); ?>"
                                                    class="um-link"><?php _e('View profile', 'ultimatemember'); ?></a>
            </div>
        </div>
...
function um_account_user_photo_hook($args)
{
    ...
    <div class="um-account-meta radius-<?php echo um_get_option('profile_photocorner'); ?>">
        ...
        <div class="um-account-meta-img uimob800-hide"><a
                href="<?php echo um_user_profile_url(); ?>"><?php echo get_avatar(um_user('ID'), 120); ?></a></div>

        <?php if ($ultimatemember->mobile->isMobile()) { ?>

            <div class="um-account-meta-img-b uimob800-show" title="<?php echo um_user('display_name'); ?>"><a
                    href="<?php echo um_user_profile_url(); ?>"><?php echo get_avatar(um_user('ID'), 120); ?></a></div>

        <?php } else { ?>

            <div class="um-account-meta-img-b uimob800-show um-tip-w" title="<?php echo um_user('display_name'); ?>"><a
                    href="<?php echo um_user_profile_url(); ?>"><?php echo get_avatar(um_user('ID'), 120); ?></a></div>
        ...
        foreach ($ultimatemember->account->tabs as $k => $arr) {
            foreach ($arr as $id => $info) {
                ...
                if (isset($info['custom']) || um_get_option('account_tab_' . $id) == 1 || $id == 'general') { ?>
                    ...
                    <li>
                        <a data-tab="<?php echo $id; ?>" href="<?php echo $ultimatemember->account->tab_link($id); ?>"
                           class="um-account-link <?php if ($id == $current_tab) echo 'current'; ?>">

                            <?php if ($ultimatemember->mobile->isMobile()) { ?>
                                <span class="um-account-icontip uimob800-show" title="<?php echo $title; ?>"><i
                                        class="<?php echo $icon; ?>"></i></span>
                            <?php } else { ?>
                                <span class="um-account-icontip uimob800-show um-tip-w" title="<?php echo $title; ?>"><i
                                        class="<?php echo $icon; ?>"></i></span>
                            <?php } ?>
                            ...
                            <span
                                class="um-account-arrow uimob800-hide"><?php echo (is_rtl()) ? '<i class="um-faicon-angle-left"></i>' : '<i class="um-faicon-angle-right"></i>'; ?></span>
                        </a>
                    </li>
