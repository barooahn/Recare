    ...
    if ($args['search'] == 1 && is_array($search_filters)) { // search on
        ...
        <div class="um-search um-search-<?php echo $count; ?>">
            ...
            foreach ($search_filters as $filter) {
                ...
                echo '<div class="um-search-filter ' . $add_class . '">';
                $ultimatemember->members->show_filter($filter);
                echo '</div>';
            ...
            <div class="um-search-submit">
                ...
                <a href="#" class="um-button um-do-search"><?php _e('Search', 'ultimatemember'); ?></a><a
                    href="<?php echo $ultimatemember->permalinks->get_current_url(true); ?>"
                    class="um-button um-alt"><?php _e('Reset', 'ultimatemember'); ?></a>
...
function um_members_directory_head($args)
{
    ...
    if (um_members('header') && $is_filtering && um_members('users_per_page')) { ?>
        ...
        <div class="um-members-intro">
            ...
            <div
                class="um-members-total"><?php echo (um_members('total_users') > 1) ? um_members('header') : um_members('header_single'); ?></div>
    ...
    if (um_members('total_pages') > 1) { // needs pagination
            ...
            <?php if (um_members('pages_to_show') && is_array(um_members('pages_to_show'))) { ?>
                <select onChange="window.location.href=this.value" class="um-s1" style="width: 100px">
                    <?php foreach (um_members('pages_to_show') as $i) { ?>
                        <option
                            value="<?php echo $ultimatemember->permalinks->add_query('members_page', $i); ?>" <?php selected($i, um_members('page')); ?>><?php printf(__('%s of %d', 'ultimatemember'), $i, um_members('total_pages')); ?></option>
                    <?php } ?>
                </select>
            <?php } ?>
        ...
        <div class="um-members-pagi uimob340-hide uimob500-hide">
            ...
            <?php if (um_members('page') != 1) { ?>
                <a href="<?php echo $ultimatemember->permalinks->add_query('members_page', 1); ?>"
                   class="pagi pagi-arrow um-tip-n" title="<?php _e('First Page', 'ultimatemember'); ?>"><i
                        class="um-faicon-angle-double-left"></i></a>
            <?php } else { ?>
                <span class="pagi pagi-arrow disabled"><i class="um-faicon-angle-double-left"></i></span>
            <?php } ?>

            <?php if (um_members('page') > 1) { ?>
                <a href="<?php echo $ultimatemember->permalinks->add_query('members_page', um_members('page') - 1); ?>"
                   class="pagi pagi-arrow um-tip-n" title="<?php _e('Previous', 'ultimatemember'); ?>"><i
                        class="um-faicon-angle-left"></i></a>
            <?php } else { ?>
            ...
            <?php if (um_members('pages_to_show') && is_array(um_members('pages_to_show'))) { ?>
                <?php foreach (um_members('pages_to_show') as $i) { ?>
                    ...
                    <?php } else { ?>
                        ...
                        <a href="<?php echo $ultimatemember->permalinks->add_query('members_page', $i); ?>"
                           class="pagi"><?php echo $i; ?></a>
            ...
            <?php if (um_members('page') != um_members('total_pages')) { ?>
                <a href="<?php echo $ultimatemember->permalinks->add_query('members_page', um_members('page') + 1); ?>"
                   class="pagi pagi-arrow um-tip-n" title="<?php _e('Next', 'ultimatemember'); ?>"><i
                        class="um-faicon-angle-right"></i></a>
            <?php } else { ?>
                <span class="pagi pagi-arrow disabled"><i class="um-faicon-angle-right"></i></span>
            <?php } ?>

            <?php if (um_members('page') != um_members('total_pages')) { ?>
                <a href="<?php echo $ultimatemember->permalinks->add_query('members_page', um_members('total_pages')); ?>"
                   class="pagi pagi-arrow um-tip-n" title="<?php _e('Last Page', 'ultimatemember'); ?>"><i
                        class="um-faicon-angle-double-right"></i></a>
            <?php } else { ?>
