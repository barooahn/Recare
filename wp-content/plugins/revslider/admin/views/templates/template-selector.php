    ...
    <div id="template_area">
        <div id="template_header_part">
            <h2><span class="revlogo-mini"
                      style="margin-right:15px;"></span><?php _e('Slider Template Library', 'revslider'); ?></h2>

            <div id="close-template"></div>

            <div class="revolution-template-switcher">
                <span data-showgroup="revolution-basic-templates"
                      class="revolution-templatebutton selected"><?php _e('Revolution Base', 'revslider'); ?></span>
                <span data-showgroup="revolution-all-slides-templates"
                      class="revolution-templatebutton"><?php _e('All Slides', 'revslider'); ?></span>
                <span data-showgroup="revolution-customer-templates" class="revolution-templatebutton"
                      style="border-right:none"><?php _e('Templates', 'revslider'); ?></span>
                <span data-showgroup="revolution-premium-templates"
                      class="revolution-templatebutton premium-templatebutton"><i
                        class="eg-icon-basket"></i><?php _e('Premium Sliders', 'revslider'); ?></span>
                <span class="rs-reload-shop"><i
                        class="eg-icon-arrows-ccw"></i><?php _e('Check for new Templates', 'revslider'); ?></span>
            </div>
            ...
            <div class="revolution-template-subtitle"><?php _e('Add Single Slide', 'revslider'); ?>
                <div class="revolution-filters" style="display: inline-block;">
                    ...
                    <span class="template_filter_button selected"
                          data-type="temp_all"><?php _e('SHOW ALL', 'revslider'); ?></span>
                    <span class="template_filter_button"
                          data-type="temp_slider"><?php _e('SLIDER', 'revslider'); ?></span>
                    <span class="template_filter_button"
                          data-type="temp_carousel"><?php _e('CAROUSEL', 'revslider'); ?></span>
                ...
                foreach ($tp_template_slider as $m_slider) {
                    ...
                    if (!isset($m_slider['installed']) && !isset($m_slider['is_new'])) {
                        ...
                        <div style="margin-bottom:30px"
                             class="template_group_wrappers <?php if (isset($m_slider['filter'])) {
                        ...
                        if (!empty($c_slides)) {
                            ?>
                        <div style="margin-bottom:30px"
                             class="template_group_wrappers not-imported-wrapper <?php if (isset($m_slider['filter'])) {
                                 echo implode(' ', $m_slider['filter']);
                             } ?>">
            ...
            if (!empty($tp_template_slider)) {
                foreach ($tp_template_slider as $m_slider) {
                    ...
                    if (!isset($m_slider['installed']) && !isset($m_slider['is_new'])) {
                        ...
                        if (!empty($c_slides)) {
                            ?>
                        <div class="template_group_wrappers <?php if (isset($m_slider['filter'])) {
                            echo implode(' ', $m_slider['filter']);
                        } ?>">
                            <?php
                            echo '<div class="template_slider_title">';
                        ...
                        if (!empty($c_slides)) {
                            ?>
                        <div class="template_group_wrappers not-imported-wrapper <?php if (isset($m_slider['filter'])) {
                            echo implode(' ', $m_slider['filter']);
                        } ?>">
            ...
            if (!empty($all_slider)) {
                foreach ($all_slider as $c_slider) {
                    ...
                    if (!empty($c_slides)) {
                        ?>
                    <div
                        class="template_group_wrappers <?php //if(isset($c_slider['filter'])){ echo implode(' ', $c_slider['filter']); }
                        ?>">
                        <?php
...
if (!isset($rs_disable_template_script)) {
    ...
    <div id="dialog_import_template_slide" title="<?php _e("Import Template Slide", 'revslider'); ?>"
        ...
        <form action="<?php echo RevSliderBase::$url_ajax; ?>" enctype="multipart/form-data" method="post">
            ...
            <p><?php _e('Please select the corresponding zip file from the download packages import folder called', 'revslider'); ?>
                :</p>

            <p class="filetoimport"><b><span class="rs-zip-name"></span></b></p>

            <p class="import-file-wrapper"><input type="file" size="60" name="import_file" class="input_import_slider">
            </p>
            <span style="margin-top:45px;display:block"><input type="submit"
                                                               ...
                                                               value="<?php _e("Import Template Slide", 'revslider'); ?>"></span>
            <span class="tp-clearfix"></span>
            <span
                style="font-weight: 700;"><?php _e("Note: style templates will be updated if they exist!", 'revslider'); ?></span><br><br>
    ...
    <div id="dialog_import_template_slide_from" title="<?php _e("Import Template Slide", 'revslider'); ?>"
        ...
        <form action="<?php echo RevSliderBase::$url_ajax; ?>" enctype="multipart/form-data" method="post"
              name="rs-import-slide-template-from-server" id="rs-import-slide-template-from-server">
            <input type="hidden" name="action" value="revslider_ajax_action">
