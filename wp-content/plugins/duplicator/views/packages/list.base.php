...
<style>
    ...
    input#dup-bulk-action-all {
        margin: 0px;
        padding: 0px 0px 0px 5px;
    }

    button.dup-button-selected {
        border: 1px solid #000 !important;
        background-color: #dfdfdf !important;
    }

    div.dup-details-area-error {
        padding: 10px;
    ...
    table.dup-pack-table {
        word-break: break-all;
    }

    table.dup-pack-table th {
        white-space: nowrap !important;
    }

    table.dup-pack-table td.pack-name {
        text-overflow: ellipsis;
        white-space: nowrap
    }

    table.dup-pack-table input[name="delete_confirm"] {
        margin-left: 15px
    }

    table.dup-pack-table td.fail {
        border-left: 4px solid #d54e21;
    }

    table.dup-pack-table td.pass {
    ...
    tr.dup-pack-info td.get-btns {
        text-align: right;
        padding: 3px 5px 6px 0px !important;
    }
...
<form id="form-duplicator" method="post">
    ...
    <?php if ($statusCount >= 3)  : ?>
        <div style="font-size:13px; position: absolute; top:10px; right:20px">
            <a href="admin.php?page=duplicator-about" style="color:maroon"><i><i
                        class="fa fa-check-circle"></i> <?php _e("Help Support Duplicator", 'duplicator') ?></i> </a>
        </div>
    <?php endif; ?>
            ...
            <td style="white-space: nowrap">
                <div class="alignleft actions">
                    <select id="dup-pack-bulk-actions">
                        ...
                        <option value="delete"
                                title="<?php _e("Delete selected package(s)", 'duplicator') ?>"><?php _e("Delete", 'duplicator') ?></option>
                    </select>
                    <input type="button" id="dup-pack-bulk-apply" class="button action"
                           value="<?php _e("Apply", 'duplicator') ?>" onclick="Duplicator.Pack.Delete()">
                </div>
                <br class="clear">
            </td>
            <td align="center">
                <a href="?page=duplicator-tools" id="btn-logs-dialog" class="button"
                   title="<?php _e("Package Logs", 'duplicator') ?>..."><i class="fa fa-list-alt"></i>
            </td>
            <td class="dup-toolbar-btns">
                <span><i class="fa fa-archive"></i> <?php _e("All Packages", 'duplicator'); ?></span> &nbsp;
                <a id="dup-pro-create-new" href="?page=duplicator&tab=new1"
                   class="add-new-h2"><?php _e("Create New", 'duplicator'); ?></a>
            </td>
        </tr>
    </table>
    ...
    <?php if ($totalElements == 0)  : ?>
        ...
        <table class="widefat dup-pack-table">
            ...
            <tbody>
            <tr>
                <td><?php include(DUPLICATOR_PLUGIN_PATH . "views/packages/list-nodata.php") ?> </td>
            </tr>
            </tbody>
            <tfoot>
            <tr>
    ...
    <?php else : ?>
        ...
        <table class="widefat dup-pack-table">
            <thead>
            <tr>
                <th><input type="checkbox" id="dup-bulk-action-all"
                           title="<?php _e("Select all packages", 'duplicator') ?>" style="margin-left:15px"
                           onclick="Duplicator.Pack.SetDeleteAll()"/></th>
            ...
            foreach ($rows as $row) {
                ...
                <?php if ($row['status'] >= 100) : ?>
                    <tr class="dup-pack-info <?php echo $css_alt ?>">
                        <td class="pass"><input name="delete_confirm" type="checkbox" id="<?php echo $row['id']; ?>"/>
                        </td>
                        <td><a href="javascript:void(0);"
                               onclick="return Duplicator.Pack.ToggleDetail('<?php echo $detail_id; ?>');">[<?php echo __("View", 'duplicator') . ' ' . $row['id']; ?>
                                ]</a></td>
                        ...
                        <td class="get-btns">
                            <button id="<?php echo "{$uniqueid}_installer.php" ?>" class="button no-select"
                                    onclick="Duplicator.Pack.DownloadFile('<?php echo $installfilelink; ?>', this); return false;">
                                <i class="fa fa-bolt"></i> <?php _e("Installer", 'duplicator') ?></button>
                            &nbsp;
                        </td>
                        <td class="get-btns">
                            <button id="<?php echo "{$uniqueid}_archive.zip" ?>" class="button no-select"
                                    onclick="Duplicator.Pack.DownloadFile('<?php echo $packagepath; ?>', this); return false;">
                                <i class="fa fa-file-archive-o"></i> <?php _e("Archive", 'duplicator') ?></button>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="8" id="<?php echo $detail_id; ?>" class="dup-pack-details  <?php echo $css_alt ?>">
                            ...
                            <button class="button"
                                    onclick="Duplicator.Pack.ShowLinksDialog(<?php echo "'{$sqlfilelink}', '{$packagepath}', '{$installfilelink}', '{$logfilelink}', '{$reportfilelink}' "; ?>); return false;"
                                    class="thickbox"><i class="fa fa-lock"></i>
                                &nbsp; <?php _e("Links", 'duplicator') ?></button>
                            &nbsp;
                            <button class="button"
                                    onclick="window.open(<?php echo "'{$sqlfilelink}', '_blank'"; ?>); return false;"><i
                                    class="fa fa-table"></i> &nbsp; <?php _e("SQL", 'duplicator') ?></button>
                            &nbsp;
                            <button class="button"
                                    onclick="Duplicator.OpenLogWindow(<?php echo "'{$logfilename}'"; ?>); return false;">
                                <i class="fa fa-list-alt"></i> &nbsp; <?php _e("Log", 'duplicator') ?></button>
                            <?php if ($package_debug) : ?>
                                <div style="margin-top:7px">
                                    <a href="javascript:void(0)"
                                       onclick="window.open(<?php echo "'{$reportfilelink}', '_blank'"; ?>); return false;">[<?php _e("Open Scan Report", 'duplicator') ?>
                                        ]</a> &nbsp;
                                    <a href="javascript:void(0)"
                                       onclick="jQuery(this).parent().find('.dup-pack-debug').toggle()">[<?php _e("View Package Object", 'duplicator') ?>
                                        ]</a><br/>
                ...
                <?php else : ?>
                    ...
                    ?>
                    <tr class="dup-pack-info  <?php echo $css_alt ?>">
                        <td class="fail"><input name="delete_confirm" type="checkbox" id="<?php echo $row['id']; ?>"/>
                        </td>
                        <td><a href="javascript:void(0);"
                               onclick="return Duplicator.Pack.ToggleDetail('<?php echo $detail_id; ?>');">[<?php echo __("View", 'duplicator') . ' ' . $row['id']; ?>
                                ]</a></td>
						...
						<span style='display:inline-block; padding:7px 40px 0px 0px'>
							<a href="javascript:void(0);"
                               onclick="return Duplicator.Pack.ToggleDetail('<?php echo $detail_id; ?>');"><?php _e("View Error Details", 'duplicator') ?>
                                ...</a>
						</span>
                        </td>
                    </tr>
                        ...
                        <td colspan="8" id="<?php echo $detail_id; ?>" class="dup-pack-details  <?php echo $css_alt ?>">
                            <div class="dup-details-area-error">
                                ...
                                <?php if ($pack_name == 'unknown') : ?>
                                    <b><?php _e("Unrecoverable Error! Please remove this package.", 'duplicator') ?></b>
                                    <br/>
                                <?php else : ?>
                                    ...
                                    <div style="height:7px">&nbsp;</div>
                                    <button class='button'
                                            onclick="Duplicator.OpenLogWindow(<?php echo "'{$logfilename}'"; ?>); return false;"><?php _e("View Log", 'duplicator') ?></button>
                                    <?php if ($package_debug) : ?>
                                        <div style="margin-top:7px">
                                            <a href="javascript:void(0)"
                                               onclick="jQuery(this).parent().find('.dup-pack-debug').toggle()">[View
                                                Package Object]</a><br/>
                                            <textarea class="dup-pack-debug"><?php print_r($Package); ?> </textarea>
                                        </div>
                                    <?php endif; ?>
...
<div id="dup-dlg-quick-path" title="<?php _e('Download Links', 'duplicator'); ?>" style="display:none">
    ...
    <div style="padding: 0px 15px 15px 15px;">
        <a href="javascript:void(0)" style="display:inline-block; text-align:right"
           onclick="Duplicator.Pack.GetLinksText()">[Select All]</a> <br/>
        <textarea id="dup-dlg-quick-path-data"
                  style='border:1px solid silver; border-radius:3px; width:99%; height:225px; font-size:11px'></textarea><br/>
...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        ...
        Duplicator.Pack.GetLinksText = function () {
            $('#dup-dlg-quick-path-data').select();
        };
